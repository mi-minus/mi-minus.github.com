...
```sh
    1. 第一步: 本机生成rsa密钥，并拷贝id_rsa.pub公共密钥内容
    2. 第二步: 将生成的公共密钥内容拷贝到 "bitbucket设置" -> "SSH密钥"中添加一项的空白框中
    3. 第三步: 代码库中的链接有两类:http 和 ssh ,此时选择使用ssh 的下拉请求(just like: git@bitbucket.org:xxxxx/xxxxx.git) 下拉即可
    4. 第四步: 保证下拉的代码库中的 .git/conf 文件中 url 等于 ssh的下拉请求(若为http请求则会被要求输入密码)
    5. 第五步: 这样就完成了免密码登陆
```
