...
5. 注意事项
```
1. mongo的复制集主要是两类，第一类是shard-replset（多个复制集，每个复制集是包含奇数台服务器的集群）,第二类是config-replset（一个复制集，其中包括奇数台服务器的集群）, 并且每个复制集中必须有一台作为primary，其他为second,为了保证投票机制设定奇数台服务器，可以建立不用存数据的arbiter作为投票服务器。复制集的初始化是分别进入到shardsrv 、 configsrv中进行初始化，这些初始化都是在建立mongos之前进行的。具体配置可参考下面：
//初始化复制集配置
//复制集节点配置说明
//主节点:priority>=1
...
rs.initiate(config);
rs.status();
2. mongo除了1中提及的复制集还包含shard-cluster集群(不分奇偶数)，每个shard是一个复制集（多台服务器）, 这一步需要建立在所有复制集都建立好之后才进行（无论分片复制集还是配置复制集），这一步addShard的操作是在mongos里进行。
