...
sh.shardCollection("cdr.register", {'$yearmon':1,'$doct.id':1, 'reg_date':1})  // $yearmon
```
>
...
sh.shardCollection('cdr.clinic_diagnose', {'$yearmon':1, '$doct.id':1, 'diag_date':1})
```
>
...
sh.shardCollection('cdr.clinic_fee_detail', {'$yearmon':1, '$doct.id':1, 'fee.date':1})
```
>
...
sh.shardCollection('cdr.clinic_medical_order', {'$yearmon':1, '$doct.id':1, 'oper_date':1}) 
```
>
...
sh.shardCollection('cdr.inpatient_mo_exec_drug', {'$yearmon':1, 'doct.id':1, 'mo_date':1}) 
```
>
...
sh.shardCollection('cdr.inpatient_mo_exec_undrug', {'$yearmon':1, '$doct.id':1, 'mo_date':1}) 
```
>
...
sh.shardCollection('cdr.inpatient_info', {'$yearmon':1, '$doct.id':1, 'in_date':1})  // in_date 不为null, out_date　有部分为null
```
>
...
sh.shardCollection('cdr.inpatient_diagnose', {'_id.inpatient_id':1, '_id.seq_no':1}) 
```
>
...
sh.shardCollection('cdr.inpatient_operation', {'$yearmon':1, 'doct_id':1, 'ops_date':1}) 
```
>
...
sh.shardCollection('cdr.inpatient_ops_arrange', {'$yearmon':1, '_id.role_id':1, 'oper_date':1}) 
```
>
...
sh.shardCollection('cdr.inpatient_ops_case', {'$yearmon':1, '_id.seq_no':1, 'ops_date':1}) 
```
>
...
sh.shardCollection('cdr.ais_oper_reg', {'$yearmon':1, 'oper.id':1, 'ops_date':1})
```
>
...
sh.shardCollection('cdr.lis', {'$yearmon':1, 'test_order.id':1, 'input_time':1})  //input_time没有为null
```
>
...
sh.shardCollection('cdr.pacs_inpatientno', {'$yearmon':1, 'test_order.id':1, 'input_time':1})
```
