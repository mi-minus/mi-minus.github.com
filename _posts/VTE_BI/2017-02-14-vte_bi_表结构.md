...
db.inpatient_info.createIndex({'rptmon':1, 'doct.id':1 ,'in_date':1})
> * shard-key
db.createCollection('inpatient_info')
sh.shardCollection('cdr.inpatient_info', {'rptmon':1, '$doct.id':1, 'in_date':1})  //$yearmon : in_date in_date 不为null, out_date　有部分为null
```
>
...
db.inpatient_operation.createIndex({'rptmon':1, 'doct_id':1, 'ops_date':1})
> * shard-key
db.createCollection('inpatient_operation')
sh.shardCollection('cdr.inpatient_operation', {'rptmon':1, 'doct_id':1, 'ops_date':1})  //$yearmon : ops_date
```
>
